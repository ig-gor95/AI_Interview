...
class EvaluationCriterion(BaseModel):
    """Evaluation criterion for candidate"""
    text: str
    is_required: bool = Field(
        alias="isRequired",
        description="True - обязательное требование, False - желательное"
    )
...
class SessionParams(BaseModel):
    ...
    must_have_requirements: Optional[List[str]] = Field(
        None,
        alias="mustHaveRequirements",
        description="Обязательные требования к кандидату"
    )
    nice_to_have_requirements: Optional[List[str]] = Field(
        None,
        alias="niceToHaveRequirements",
        description="Желательные требования к кандидату"
    )
    evaluation_criteria: Optional[List[str]] = Field(
        None,
        alias="evaluationCriteria",
        description="Общие критерии оценки (для обратной совместимости)"
    )
