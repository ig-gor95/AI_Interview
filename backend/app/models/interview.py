...
class InterviewEvaluationCriterion(Base):
    ...
    is_required = Column(Boolean, nullable=False, default=True)  # True = обязательное, False = желательное
    order_index = Column(Integer, nullable=False, default=0)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    ...
    def __repr__(self):
        return f"<InterviewEvaluationCriterion(id={self.id}, criterion={self.criterion_name}, required={self.is_required})>"
