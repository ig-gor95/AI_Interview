...
# Скрипт обновления приложения на Timeweb Cloud
# Использование: ./update-timeweb.sh [--no-build] [--backend-only] [--frontend-only]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Использование: ./update-timeweb.sh [--no-build] [--backend-only] [--frontend-only]"
            exit 1
            ;;
    esac
done
...
./backup-timeweb.sh 2>/dev/null || echo -e "${YELLOW}⚠️  Бэкап не создан${NC}"
...
if [ "$NO_BUILD" = false ]; then
    ...
    for service in $SERVICES; do
        docker-compose -f docker-compose.timeweb.yml build $service
    done
else
...
docker-compose -f docker-compose.timeweb.yml stop $SERVICES

# Запуск миграций (только для backend)
if [[ "$SERVICES" == *"backend"* ]]; then
    echo "📊 Запуск миграций базы данных..."
    docker-compose -f docker-compose.timeweb.yml run --rm backend alembic upgrade head 2>/dev/null || {
...
docker-compose -f docker-compose.timeweb.yml up -d $SERVICES
...
for service in $SERVICES; do
    echo ""
    echo "--- Логи $service ---"
    docker-compose -f docker-compose.timeweb.yml logs --tail=30 $service
done
...
if [ "$SUCCESS" = true ]; then
    ...
    echo "Проверьте логи: docker-compose -f docker-compose.timeweb.yml logs"
    exit 1
fi

echo ""
echo "💡 Полезные команды:"
echo "  docker-compose -f docker-compose.timeweb.yml logs -f $SERVICES    # Логи в реальном времени"
echo "  docker-compose -f docker-compose.timeweb.yml restart $SERVICES    # Перезапуск сервисов"
