...
export function SessionView({ session, onComplete, onBack }: Props) {
  ...
  const generateGreeting = () => {
    ...
    if (params.interviewType === 'screening') {
      return t.sessionView.greetingScreening.replace('{position}', position);
    }
    
    const greetings = {
      friendly: t.sessionView.greetingFriendly.replace('{topic}', params.topic || ''),
      professional: t.sessionView.greetingProfessional.replace('{topic}', params.topic || ''),
      motivating: t.sessionView.greetingMotivating.replace('{topic}', params.topic || '')
    };
  ...
  const toggleListening = () => {
    if (!isMuted && !isProcessing) {
      if (!isListening) {
        ...
        const userMessage = t.sessionView.userSampleMessage;
  ...
  const generateAIResponse = (userMessage: string) => {
    ...
    if (params.interviewType === 'screening' && params.questions && params.questions.length > 0) {
      ...
      return t.sessionView.aiFinalQuestion;
    }
    
    const topic = params.topic || '';
    const responses = [
      t.sessionView.aiResponse1,
      t.sessionView.aiResponse2.replace('{topic}', topic),
      t.sessionView.aiResponse3,
      t.sessionView.aiResponse4,
      t.sessionView.aiResponse5.replace('{topic}', topic),
      t.sessionView.aiResponse6
    ];
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
          ...
          <div className="space-y-3 mb-8">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <span className="text-sm text-gray-700">{t.sessionView.duration}</span>
              <span className="text-sm font-medium text-gray-900">{session.params.duration} {t.sessionView.minutes}</span>
            </div>
            
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <span className="text-sm text-gray-700">{t.sessionView.level}</span>
              <span className="text-sm font-medium text-gray-900">
                {session.params.difficulty === 'beginner' ? t.sessionView.beginner :
                 session.params.difficulty === 'intermediate' ? t.sessionView.intermediate : t.sessionView.advanced}
              </span>
            </div>

            {session.params.questions && session.params.questions.length > 0 && (
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <span className="text-sm text-gray-700">{t.sessionView.questionsCount}</span>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={t.sessionView.participants}
            >
            ...
            <button
              ...
              title={t.sessionView.transcript}
            >
            ...
            <button className="p-2.5 hover:bg-gray-700/50 rounded-lg transition-all text-gray-300" title={t.sessionView.settings}>
      ...
      <div className="flex-1 flex overflow-hidden relative" style={{ minHeight: 0 }}>
        ...
        <div className="flex-1 p-3 sm:p-6 flex flex-col lg:flex-row gap-3 sm:gap-6" style={{ minHeight: 0 }}>
          ...
          <div className="relative bg-black rounded-2xl overflow-hidden flex-1 shadow-2xl border border-gray-800/50 group" style={{ minHeight: 0 }}>
            ...
            <div className="absolute bottom-4 left-4 px-4 py-2.5 bg-black/80 backdrop-blur-md rounded-xl flex items-center gap-3 z-10 border border-white/10">
              ...
              <div>
                <span className="text-sm text-white font-semibold">{t.sessionView.sofia}</span>
                <p className="text-xs text-gray-400">{t.sessionView.recruiter}</p>
              </div>
            </div>
            ...
            {isProcessing && (
              <div className="absolute top-4 left-4 px-4 py-2.5 bg-blue-600/90 backdrop-blur-md rounded-xl z-10 border border-blue-400/30 shadow-lg">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm text-white font-medium ml-1">{t.sessionView.analyzing}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="relative bg-black rounded-2xl overflow-hidden flex-1 shadow-2xl border border-gray-800/50 group" style={{ minHeight: 0 }}>
              ...
              <div className="relative w-48 h-48 rounded-full bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 shadow-2xl shadow-teal-500/30 flex items-center justify-center">
                ...
                <div className="relative z-10">
                  <span className="text-6xl font-light text-white/95 tracking-tight">
                    {(user?.name || t.sessionView.you).charAt(0).toUpperCase()}
                  </span>
                </div>
            ...
            <div className="absolute bottom-4 left-4 px-4 py-2.5 bg-black/80 backdrop-blur-md rounded-xl flex items-center gap-3 z-10 border border-white/10">
              ...
              <div>
                <span className="text-sm text-white font-semibold">{user?.name || t.sessionView.you}</span>
                {isMuted && (
                  <div className="flex items-center gap-1 mt-0.5">
                    <MicOff className="w-3 h-3 text-red-400" />
                    <span className="text-xs text-red-400">{t.sessionView.micOff}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {isListening && (
              <div className="absolute top-4 left-4 px-4 py-2.5 bg-red-600/90 backdrop-blur-md rounded-xl animate-pulse z-10 border border-red-400/30 shadow-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white font-semibold">{t.sessionView.recording}</span>
                </div>
              </div>
            )}
        ...
        {showChat && (
          <div className="w-80 sm:w-96 bg-[#232323] border-l border-gray-800/50 flex flex-col flex-shrink-0 shadow-2xl">
            <div className="px-5 py-4 border-b border-gray-800/50 flex items-center justify-between flex-shrink-0">
              <h3 className="text-white font-semibold">{t.sessionView.dialogTranscript}</h3>
              <button
                ...
                <div key={i} className="space-y-2 animate-fadeIn">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold ${
                      ...
                      {msg.role === 'ai' ? 'AI' : (user?.name || t.sessionView.you).charAt(0).toUpperCase()}
                    </div>
                    <span className="text-xs font-semibold text-gray-300">
                      {msg.role === 'ai' ? t.sessionView.sofia : (user?.name || t.sessionView.you)}
                    </span>
        ...
        {showParticipants && (
          <div className="w-72 sm:w-80 bg-[#232323] border-l border-gray-800/50 flex flex-col flex-shrink-0 shadow-2xl">
            <div className="px-5 py-4 border-b border-gray-800/50 flex items-center justify-between flex-shrink-0">
              <div>
                <h3 className="text-white font-semibold">{t.sessionView.participants}</h3>
                <p className="text-xs text-gray-400 mt-0.5">{t.sessionView.twoParticipants}</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2" style={{ minHeight: 0 }}>
              ...
              <div className="flex items-center gap-3 p-3 hover:bg-gray-700/30 rounded-xl transition-all">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0 shadow-lg">
                  <span className="text-sm text-white font-bold">{t.sessionView.sofia.charAt(0).toUpperCase()}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-white font-semibold truncate">{t.sessionView.sofia}</div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-gray-400">{t.sessionView.recruiter}</span>
                    {isSpeaking && (
              ...
              <div className="flex items-center gap-3 p-3 hover:bg-gray-700/30 rounded-xl transition-all">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-white font-semibold truncate">{user?.name || t.sessionView.you}</div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-gray-400">{t.sessionView.candidate}</span>
                    {isListening ? (
                      <div className="flex items-center gap-0.5">
        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            ...
            >
              ...
              <span className="text-[9px] sm:text-[10px] leading-none hidden sm:block">{isMuted ? t.sessionView.turnOn : t.sessionView.mic}</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              ...
              title={
                isListening ? t.sessionView.stopRecording : 
                isMuted ? t.sessionView.enableMic :
                isProcessing ? t.sessionView.waitForResponse :
                t.sessionView.pressToAnswer
              }
            >
              <Mic className="w-5 h-5 sm:w-6 sm:h-6" />
              <span className="text-[10px] sm:text-xs font-bold tracking-wide">
                {isListening ? t.sessionView.speaking3 : t.sessionView.answer}
              </span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            ...
            >
              ...
              <span className="text-xs sm:text-sm hidden sm:inline">{t.sessionView.endCall}</span>
            </button>
          </div>
        </div>
      </div>
