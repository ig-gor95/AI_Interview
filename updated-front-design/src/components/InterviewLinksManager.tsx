...
import { ArrowLeft, Plus, Copy, Check, ExternalLink, Calendar, User, CheckCircle, Circle, Trash2, Download, X } from 'lucide-react';
import { Session, InterviewLink } from '@/types';
import { Button } from './ui/button';
import { Input } from './ui/input';
import { getInterviewLinks, createInterviewLink, deleteInterviewLink } from '@/lib/mockData';
...
export function InterviewLinksManager({ session, onBack }: Props) {
  ...
  const loadLinks = () => {
    setLinks(getInterviewLinks(session.id));
  };

  const handleGenerateLinks = (count?: number) => {
    const linksCount = count !== undefined ? count : Math.min(Math.max(1, generatingCount), 100);
    
    for (let i = 0; i < linksCount; i++) {
      createInterviewLink(session.id);
    }
