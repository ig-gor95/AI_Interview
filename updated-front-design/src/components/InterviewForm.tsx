...
export function InterviewForm({ onClose, onCreate }: Props) {
  ...
  const generateQuestions = async () => {
    ...
    // Mock AI generation - имитация генерации
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    try {
      // Простая mock-генерация на основе ключевых слов
      ...
      // Определяем тип позиции и генерируем вопросы
      if (description.includes('официант') || description.includes('бармен') || description.includes('ресторан')) {
        questions = [
        ...
        newClarifications = {
          0: ['Сколько лет вы работали в этой сфере?', 'В каком формате заведения вы работали?'],
          2: ['Как вы распределяли приоритеты между столиками?'],
          3: ['Вы когда-нибудь делали комплимент от заведения?']
        };
      } else if (description.includes('колл-центр') || description.includes('оператор') || description.includes('поддержка')) {
        questions = [
          'Расскажите о вашем опыте работы с клиентами по телефону',
          'Как вы справляетесь с агрессивными или недовольными клиентами?',
          'Опишите вашу скорость печти и навыки работы с компьютером',
          'Готовы ли вы к работе по сменому графику?'
        ];
        ...
        newClarifications = {
          0: ['Сколько звонков в день вы обрабатывали?', 'С каким CRM-системами вы работали?'],
          1: ['Расскажите конкретный пример из практики'],
          2: ['Какие программы вы используете в работе?']
        };
      } else if (description.includes('администратор') || description.includes('ресепшн') || description.includes('отель')) {
        questions = [
          'Расскажите о вашем опыте работы на ресепшн или администратором',
          'Как вы организуете свою работу при большом потоке гостей?',
          'Опишите ситуацию, когда нужно было ршить конфликт между гостями',
        ...
        newClarifications = {
          0: ['В каких системах бронирования вы работали?'],
          1: ['Какие инструменты вы используете для организации работы?'],
          3: ['Проходили ли вы языковые курсы?']
        };
      } else if (description.includes('продав') || description.includes('консультант') || description.includes('менеджер по продажам')) {
        questions = [
        ...
        newClarifications = {
          0: ['Какой был ваш средний чек?', 'Сколько сделок закрывали в месяц?'],
          1: ['Приведите пример работы с конкретным возражением'],
          3: ['Какая была сумма сделки?']
        };
      } else {
        // Общие вопросы для любой позиции
        questions = [
          'Расскажите о вашем опыте работы в этой сфере',
          'Почему вы хотите работать именно на этой позиции?',
        ...
        newClarifications = {
          0: ['Сколько лет опыта у вас в этой облати?'],
          2: ['Приведите конкретный пример из практики']
        };
      }
      ...
      alert('Произошла ошибка при генерации вопросов. Попробуйте снова.');
    } finally {
  ...
  const generateCriteriaWithAI = async () => {
    ...
    // Mock AI generation based on position
    const position = formData.position.toLowerCase();
    let mustHave: string[] = [];
    let niceToHave: string[] = [];
    
    if (position.includes('оператор') || position.includes('колл-центр')) {
      mustHave = [
    ...
    } else if (position.includes('официант') || position.includes('бармен')) {
      mustHave = [
        'Опыт работы в общественном питании от 6 месяцев',
        'Коммуникабельность и клиентоориентированность',
        'Готовность к работе в выходные и вечернее время',
    ...
    setMustHaveRequirements(mustHave);
    setNiceToHaveRequirements(niceToHave);
